...
under the same verifier-call budget; the branch-curriculum example certifies a
monotone easy-to-hard target sequence under a matched verifier-call budget; the
branch-contingency example certifies a
...
hindsight-relabel example certifies that a rejected source branch may guide a
...
evidence before reuse; the branch-recency example uses recent reject/commit
receipts to override older stale commits before a target spends its one-call
verifier budget; the branch-restart example uses source local-dead-end and
...
under the same one-call budget; the branch-confidence example uses a fixed
...
the branch-propensity-match example uses receipt-bound covariate balance to
avoid context-mismatched source replay under the same one-call budget;
the branch-robustness example uses uncertainty-set source receipts to avoid
brittle nominal source replay under the same one-call budget;
the branch-calibration example uses receipt-bound confidence bins to avoid
overconfident source replay under the same one-call budget;
the branch-conformal example uses receipt-bound nonconformity quantiles to avoid
...
one-call budget;
the branch-potential-heuristic example uses receipt-bound potential values to
avoid high-potential target proposals under the same one-call budget;
...
to avoid duplicate known-bad target branches under the same one-call budget;
...
signature without transferring commit authority; the trust-region example uses
source reject/commit radii to cap target proposal size before fresh hard
verification; the
...
forty-six branch-memory stages. See `examples/README.md` and
