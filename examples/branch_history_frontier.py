...
BRANCH_HISTORY_FRONTIER_CLAIM_BOUNDARY = (
    ...
    "distributional robustness, neural-network calibration, statistical calibration, probability "
    ...
    "finding, path-following performance, partial-order reduction algorithm, model-checking correctness "
    "proof, dynamic partial-order reduction result, concurrency verification result, state-space reduction "
    "guarantee, bifurcation analysis, branch-switching algorithm performance, "
    "sensitivity-analysis algorithm, elementary-effects screening result, SPSA gradient estimate, "
...
def build_branch_history_frontier_result(
    ...
    report = BranchHistoryFrontierReport(
        ...
        branch_calibration_certificate_count=_metric(children, "branch_calibration_certificate_count"),
        calibrated_success_count=_metric_for(children, "branch_calibration_transfer", "calibrated_success_count"),
        branch_conformal_certificate_count=_metric(children, "branch_conformal_certificate_count"),
        conformal_success_count=_metric_for(children, "branch_conformal_transfer", "conformal_success_count"),
        branch_active_subspace_certificate_count=_metric(children, "branch_active_subspace_certificate_count"),
        active_subspace_success_count=_metric_for(children, "branch_active_subspace_transfer", "active_subspace_success_count"),
        branch_sensitivity_certificate_count=_metric(children, "branch_sensitivity_certificate_count"),
        ...
        commutative_success_count=_metric_for(children, "branch_commutativity_transfer", "commutative_success_count"),
        ...
        branch_transposition_certificate_count=_metric(children, "branch_transposition_certificate_count"),
        transposition_success_count=_metric_for(children, "branch_transposition_transfer", "transposition_success_count"),
        ...
        learning=(
            ...
            "continuation transfer thirty-fifth, commutativity transfer thirty-sixth, "
            "branch-switch transfer thirty-seventh, transposition transfer thirty-eighth, "
            "receipt-bound branch pruning thirty-ninth, diversity-certified family coverage fortieth, "
            "budget-allocation transfer forty-first, no-good stop-rule abstention forty-second, "
            "branch composition forty-third, and retained-memory influence with matched ablation forty-fourth."
        ),
    )
    claim = certify_claim(
        ...
        requirements=(
            requirement("exactly_forty_four_branch_history_stages", report.stage_count == 44),
            requirement(
            ...
            requirement(
                "branch_calibration_certificates_present",
                report.branch_calibration_certificate_count == 3 and report.calibrated_success_count == 3,
            ),
            requirement(
                "branch_conformal_certificates_present",
                report.branch_conformal_certificate_count == 3 and report.conformal_success_count == 3,
            ),
            requirement(
                "branch_active_subspace_certificates_present",
                report.branch_active_subspace_certificate_count == 3 and report.active_subspace_success_count == 3,
            ),
            requirement(
                "branch_sensitivity_certificates_present",
                report.branch_sensitivity_certificate_count == 3 and report.sensitivity_success_count == 3,
            ),
            requirement(
            ...
            requirement(
                "branch_commutativity_certificates_present",
                report.branch_commutativity_certificate_count == 3 and report.commutative_success_count == 3,
            ),
            requirement(
            ...
            requirement(
                "branch_transposition_certificates_present",
                report.branch_transposition_certificate_count == 3 and report.transposition_success_count == 3,
            ),
        ...
        metrics={
            ...
            "branch_calibration_certificate_count": report.branch_calibration_certificate_count,
            "calibrated_success_count": report.calibrated_success_count,
            "branch_conformal_certificate_count": report.branch_conformal_certificate_count,
            "conformal_success_count": report.conformal_success_count,
            "branch_active_subspace_certificate_count": report.branch_active_subspace_certificate_count,
            "active_subspace_success_count": report.active_subspace_success_count,
            "branch_sensitivity_certificate_count": report.branch_sensitivity_certificate_count,
            ...
            "branch_transposition_certificate_count": report.branch_transposition_certificate_count,
...
def _primary_certificate(child: CertifiedExampleResult) -> Any:
    ...
    if experiment_id == "branch_calibration_transfer":
        return child.branch_calibration_transfer_certificate
    if experiment_id == "branch_conformal_transfer":
        return child.branch_conformal_transfer_certificate
    if experiment_id == "branch_active_subspace_transfer":
        return child.branch_active_subspace_transfer_certificate
    if experiment_id == "branch_sensitivity_transfer":
    ...
    if experiment_id == "branch_commutativity_transfer":
        return child.branch_commutativity_transfer_certificate
    if experiment_id == "branch_switch_transfer":
        return child.branch_switch_transfer_certificate
    if experiment_id == "branch_transposition_transfer":
        return child.branch_transposition_transfer_certificate
...
def _primary_certificate_valid(child: CertifiedExampleResult) -> bool:
    ...
    if experiment_id == "branch_calibration_transfer":
        return validate_branch_calibration_transfer_certificate(
            child.branch_calibration_transfer_certificate,
            child.report,
        )
    if experiment_id == "branch_conformal_transfer":
        return validate_branch_conformal_transfer_certificate(
            child.branch_conformal_transfer_certificate,
            child.report,
        )
    if experiment_id == "branch_active_subspace_transfer":
        return validate_branch_active_subspace_transfer_certificate(
            child.branch_active_subspace_transfer_certificate,
            child.report,
        )
    if experiment_id == "branch_sensitivity_transfer":
        return validate_branch_sensitivity_transfer_certificate(
            child.branch_sensitivity_transfer_certificate,
            child.report,
        )
    if experiment_id == "branch_continuation_transfer":
    ...
    if experiment_id == "branch_commutativity_transfer":
        return validate_branch_commutativity_transfer_certificate(
            child.branch_commutativity_transfer_certificate,
            child.report,
        )
    if experiment_id == "branch_switch_transfer":
    ...
    if experiment_id == "branch_transposition_transfer":
        return validate_branch_transposition_transfer_certificate(
            child.branch_transposition_transfer_certificate,
            child.report,
        )
    if experiment_id == "branch_pruning_transfer":
...
def _stage_fields(child: CertifiedExampleResult) -> tuple[str, str, str, str, bool, str]:
    ...
    if experiment_id == "branch_calibration_transfer":
        return (
            ...
            f"overconfident target commits {report.static_success_count}/{report.domain_count}",
            f"calibrated target commits {report.calibrated_success_count}/{report.domain_count}",
            f"calibration certificates {report.branch_calibration_certificate_count}",
            True,
            "confidence-bin calibration certificates before trusting proposer scores",
        )
    if experiment_id == "branch_conformal_transfer":
        return (
            "receipt_bound_nonconformity_quantile",
            f"out-of-envelope target commits {report.static_success_count}/{report.domain_count}",
            f"in-envelope target commits {report.conformal_success_count}/{report.domain_count}",
            f"conformal certificates {report.branch_conformal_certificate_count}",
            True,
            "nonconformity-quantile certificates before source replay admission",
        )
    if experiment_id == "branch_active_subspace_transfer":
        return (
            "receipt_bound_active_subspace_projection",
            f"orthogonal target commits {report.static_success_count}/{report.domain_count}",
            f"active-subspace target commits {report.active_subspace_success_count}/{report.domain_count}",
            f"active-subspace certificates {report.branch_active_subspace_certificate_count}",
            True,
            "active-direction projection certificates before low-rank proposal filtering",
        )
    if experiment_id == "branch_sensitivity_transfer":
        return (
            "receipt_bound_sensitivity_axis",
            f"wrong-direction target commits {report.static_success_count}/{report.domain_count}",
            f"sensitivity-guided target commits {report.sensitivity_success_count}/{report.domain_count}",
            f"sensitivity certificates {report.branch_sensitivity_certificate_count}",
            True,
            "sensitivity-axis certificates before wrong-direction target perturbations",
        )
    if experiment_id == "branch_continuation_transfer":
        return (
            "receipt_bound_continuation_path",
            f"direct-jump target commits {report.static_success_count}/{report.domain_count}",
            f"continuation target finals commit {report.continuation_success_count}/{report.domain_count}",
    ...
    if experiment_id == "branch_commutativity_transfer":
        return (
            ...
            f"non-canonical target commits {report.static_success_count}/{report.domain_count}",
            f"canonical-order target commits {report.commutative_success_count}/{report.domain_count}",
            f"commutativity certificates {report.branch_commutativity_certificate_count}",
            True,
            "commutativity certificates before non-canonical target order replay",
        )
    if experiment_id == "branch_switch_transfer":
        return (
    ...
    if experiment_id == "branch_transposition_transfer":
        return (
            ...
            f"duplicate target commits {report.static_success_count}/{report.domain_count}",
            f"non-duplicate target commits {report.transposition_success_count}/{report.domain_count}",
            f"transposition certificates {report.branch_transposition_certificate_count}",
            True,
            "canonical-state transposition certificates before duplicate target replay",
        )
    if experiment_id == "branch_composition_transfer":
        return (
