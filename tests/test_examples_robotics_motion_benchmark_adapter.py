...
class RoboticsMotionBenchmarkAdapterTests(unittest.TestCase):
    ...
    def test_deterministic_backend_exercises_receipt_trained_robotics_adapter_mechanics(self) -> None:
        ...
        self.assertEqual(report.train_task_ids, ("train-kitchen-pick",))
        self.assertEqual(report.held_out_task_ids, ("heldout-shelf-place", "heldout-cabinet-reach"))
        ...
        self.assertEqual(report.learner_snapshot_receipt_hashes, report.receipt_hashes[:report.training_receipt_count])
        self.assertTrue(report.learner_snapshot_row_hashes)
        self.assertTrue(report.proposer_rank_audit_ok)
        self.assertEqual(len(report.proposer_rank_audit_hashes), len(report.held_out_task_ids))
        ...
        self.assertEqual(result.evidence_certificate.learner_snapshot_receipt_hashes, report.learner_snapshot_receipt_hashes)
        self.assertEqual(result.evidence_certificate.learner_snapshot_row_hashes, report.learner_snapshot_row_hashes)
        self.assertEqual(result.evidence_certificate.proposer_rank_audit_hashes, report.proposer_rank_audit_hashes)
    ...
    def test_deterministic_backend_cannot_support_real_robotics_claim(self) -> None:
        result = run_robotics_motion_benchmark_adapter_experiment(DeterministicMotionBenchmarkBackend())

        self.assertTrue(validate_claim_certificate(result.claim_certificate))
        self.assertEqual(result.claim_certificate.status, "rejected")
        self.assertEqual(result.claim_certificate.evidence_grade, "G0")
        self.assertIn("real_motion_benchmark_backend", result.claim_certificate.failed_keys)
        ...
        self.assertIn("Single-domain robotics adapter evidence only", result.claim_certificate.boundary)

    def test_default_backend_reports_missing_roslaunch_or_task_root_or_real_backend(self) -> None:
        backend = MotionBenchmarkBackend(timeout_seconds=1)
        result = run_robotics_motion_benchmark_adapter_experiment(backend)

        self.assertEqual(result.report.backend_id, "motionbenchmaker+moveit+ompl")
        self.assertTrue(validate_claim_certificate(result.claim_certificate))
    ...
    def test_motion_benchmark_acceptance_requires_explicit_clearance(self) -> None:
        result = {
            "solved": True,
        ...
        self.assertFalse(_benchmark_result_accepted(result))
        self.assertEqual(_benchmark_residual_kind(result), "motion_benchmark_clearance_missing")

        with_clearance = dict(result)
        with_clearance["solution_clearance"] = 0.0
        self.assertTrue(_benchmark_result_accepted(with_clearance))
