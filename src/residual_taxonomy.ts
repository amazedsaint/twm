...
import { TransactionEngine, makeTrace, type Receipt } from "./core.js";
import {
...
export interface ResidualTaxonomyReport {
  signalCount: number;
  ...
  stockShortageFields: string[];
  projectionMissingFields: string[];
  budgetFields: string[];
  topStockShortageRepairHint: string;
  snakeCamelLearningHashEqual: boolean;
  rawResidualHashDistinct: boolean;
  tamperDetected: boolean;
  ledgerAudit: boolean;
  invalidCommitCount: number;
}

export async function runResidualTaxonomyBenchmark(): Promise<ResidualTaxonomyReport> {
  ...
  const tampered = { ...signals[0], category: "unknown" as const };
...
async function stockShortageReceipt(): Promise<[Receipt, boolean, number]> {
  const state: InventoryState = { stock: { widget: 2 }, reserved: {}, committedOrders: [] };
...
async function makeProjectionResidualReceipt(): Promise<[Receipt, boolean, number]> {
  const state: ProjectionGuardState = { distanceToObstacle: 5, brakeAccel: 2, safetyClearance: 2, committedModes: [] };
...
async function budgetAbstainReceipt(): Promise<[Receipt, boolean, number]> {
  ...
  return [outcome.receipts[0] as Receipt, await engine.ledger.audit(), engine.invalidCommitCount];
}
