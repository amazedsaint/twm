import { ReceiptBudgetPolicy, type BudgetCandidate, validateBudgetPolicySnapshot } from "./budget_policy.js";
...
export const BUDGET_POLICY_ORDER = [8, 7, 5, 4] as const;

export interface BudgetPolicyReport {
  trainingReceiptCount: number;
  budget: number;
  candidateCount: number;
  learnedSuccessToken: string;
  learnedSuccessLowerBound: number;
  cheapFirstSelected: string[];
  cheapFirstCommitted: boolean;
  cheapFirstVerifierCalls: number;
  cheapFirstCostSpent: number;
  learnedSelected: string[];
  learnedCommitted: boolean;
  learnedCommittedLabel: string;
  learnedVerifierCalls: number;
  learnedCostSpent: number;
  learnedExpectedUtility: number;
  verifierCallGain: number;
  verifierCostRatio: number;
  evaluationReceiptCount: number;
  heldoutTraceDisjoint: boolean;
...
export async function runBudgetPolicyBenchmark(): Promise<BudgetPolicyReport> {
  const seedState: InventoryState = { stock: { widget: 5 }, reserved: { widget: 0 }, committedOrders: [] };
  const policy = new ReceiptBudgetPolicy();
  const trainingEngine = new TransactionEngine(new InventoryReservationAdapter());
  const trainingSpecs: Array<[string, number]> = [["quantity-5", 5], ["quantity-8", 8], ["quantity-7", 7]];
  const trainingBranchIds: string[] = [];
...
async function budgetCandidates(state: InventoryState): Promise<Array<BudgetCandidate<InventoryReservationPayload>>> {
...
async function cheapFirstSubmit(
  ...
  budget: number,
): Promise<{ committed: boolean; submittedLabels: string[]; verifierCostSpent: number }> {
  let spent = 0;
