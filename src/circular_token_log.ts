...
export interface CircularTokenLogReport {
  ...
  retainedReplayTokenCount: number;
  replayTokensSaved: number;
  compactedTokenSummary: string[];
  suffixTokenSummary: string[];
  fullState: Record<string, unknown>;
  compactedState: Record<string, unknown>;
  fullEqualsCompacted: boolean;
...
export async function runCircularTokenLogBenchmark(): Promise<CircularTokenLogReport> {
...
export function demoCircularLogState(): Record<string, number> {
  return { a: 0, b: 0, c: 0, d: 0 };
}

export function demoCircularLogTokens(): DeltaToken[] {
  return [
...
function tokenSummary(token: DeltaToken): string {
