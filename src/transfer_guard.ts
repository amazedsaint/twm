import { type BudgetCandidate, ReceiptBudgetPolicy } from "./budget_policy.js";
...
export interface TransferGuardReport {
  schemaVersion: string;
  snapshotValid: boolean;
  decisionValid: boolean;
  guardBlocksSourcePolicy: boolean;
  guardDecisionAdmitted: boolean;
  guardDecisionReason: string;
  guardDecisionHash: string;
  sourceSelected: string[];
  unguardedSelected: string[];
  unguardedCommitted: boolean;
  unguardedResidualKind: string;
  guardedSelected: string[];
  guardedCommitted: boolean;
  guardedUsedTargetBaseline: boolean;
  avoidedNegativeTransfer: boolean;
  certificateConclusion: string;
...
export async function runTransferGuardBenchmark(): Promise<TransferGuardReport> {
  const sourceState: InventoryState = { stock: { widget: 5 }, reserved: { widget: 0 }, committedOrders: [] };
  const targetState: InventoryState = { stock: { widget: 2 }, reserved: { widget: 0 }, committedOrders: [] };
  ...
  let guardedSelected: string[];
  let guardedCommitted: boolean;
  let guardedUsedTargetBaseline: boolean;
  ...
  const unguardedResidualKind = unguardedResidual && typeof unguardedResidual === "object"
    ? String((unguardedResidual as { kind?: unknown }).kind ?? "")
    : "";
  const engines: Array<[TransactionEngine<InventoryState, InventoryReservationPayload>, InventoryState]> = [
    [sourceEngine, sourceState],
    [evalTransferEngine, targetState],
    [evalBaselineEngine, targetState],
...
async function targetCandidates(state: InventoryState): Promise<Array<BudgetCandidate<InventoryReservationPayload>>> {
...
async function targetLocalSubmit(
  ...
  budget: number,
): Promise<{ committed: boolean; submittedLabels: string[]; receipts: Receipt[] }> {
