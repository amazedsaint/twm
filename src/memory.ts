...
export interface MacroMemoryEntry<Step = unknown> {
...
export interface MacroMemorySnapshot<Step = unknown> {
  schemaVersion: string;
  capacityPerContext: number;
  totalUpdates: number;
  evictedCount: number;
  entries: Array<MacroMemoryEntry<Step>>;
  snapshotHash: string;
}

interface MutableMacroEntry<Step> {
  context: string;
  token: string;
  macroSteps: Step[];
  acceptedCount: number;
...
export class BoundedMacroMemory<Step = unknown> {
  capacityPerContext: number;
  stalenessPenalty: number;
  totalUpdates = 0;
  evictedCount = 0;
  private entries = new Map<string, MutableMacroEntry<Step>>();

  constructor(capacityPerContext = 16, options: { stalenessPenalty?: number } = {}) {
  ...
  async update(receipt: Receipt): Promise<void> {
    ...
    const macroSteps = macroStepsFromPayload<Step>(payload);
  ...
  async score(context: string, macro: Macro<Step>): Promise<number> {
    const entry = this.entries.get(entryKey(context, await macroToken(macro.steps)));
    return entry ? entryScore(entry) : 0;
  }

  async rank(context: string, macros: Array<Macro<Step>>): Promise<Array<Macro<Step>>> {
  ...
  async snapshot(): Promise<MacroMemorySnapshot<Step>> {
    ...
    const snapshot: MacroMemorySnapshot<Step> = {
  ...
  private evictOverCapacity(context: string): void {
...
export async function macroToken(steps: unknown[]): Promise<string> {
  return stableHash(steps);
}

export async function macroMemoryEntryHash(entry: MacroMemoryEntry | Record<string, unknown>): Promise<string> {
  const { entryHash: _entryHash, ...withoutHash } = entry as Record<string, unknown>;
  return stableHash(withoutHash);
}

export async function macroMemorySnapshotHash(snapshot: MacroMemorySnapshot | Record<string, unknown>): Promise<string> {
  const { snapshotHash: _snapshotHash, ...withoutHash } = snapshot as Record<string, unknown>;
  return stableHash(withoutHash);
}

export async function validateMacroMemorySnapshot(snapshot: MacroMemorySnapshot | Record<string, unknown>): Promise<boolean> {
  try {
...
async function macroMemoryEntrySnapshot<Step>(
  entry: MutableMacroEntry<Step>,
  currentIndex: number,
  stalenessPenalty: number,
): Promise<MacroMemoryEntry<Step>> {
  const snapshot: MacroMemoryEntry<Step> = {
...
function entryScore(entry: MutableMacroEntry<unknown>): number {
  return 4 * entry.acceptedCount - 3 * entry.prefixRejectCount - 2 * entry.terminalRejectCount;
}

function retentionPriority(entry: MutableMacroEntry<unknown>, currentIndex: number, stalenessPenalty: number): number {
...
function entryKey(context: string, token: string): string {
  return `${context}\n${token}`;
}
...
function macroStepsFromPayload<Step>(payload: Record<string, unknown>): Step[] {
  return Array.isArray(payload.macro) ? payload.macro as Step[] : [];
}
