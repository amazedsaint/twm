...
export type CodeOperator = "left" | "+" | "-" | "*" | "max" | "min" | "right" | "absdiff";

export const DEFAULT_CODE_OPERATORS: CodeOperator[] = ["left", "+", "-", "*", "max", "min", "right", "absdiff"];
export const INITIAL_CODE_OPERATOR: CodeOperator = "left";
export const BASE_SOURCE = "def combine(x, y):\n    return __op0__(x, y)\n";
export const DEFAULT_TEST_INPUTS: Array<Record<string, number>> = [
  { x: 0, y: 0 },
  { x: 1, y: 2 },
  { x: 3, y: 1 },
  { x: -2, y: 5 },
...
export interface CodePatchProblem {
  filePath: string;
  functionName: string;
  baseSource: string;
  sourceHash: string;
  tests: CodeTestCase[];
  mutableNodeId: string;
  leftVar: string;
  rightVar: string;
  allowedOperators: CodeOperator[];
}
...
export interface CodeRepairState {
...
export interface CodeResidual {
  kind: "schema_error" | "test_failure";
  message?: string;
  testIndex?: number;
  inputs?: Record<string, number>;
  expected?: number;
...
export interface CodeRepairEpisodeResult {
  calls: number;
  success: boolean;
  auditOk: boolean;
...
export class CodePatchAdapter implements ReplayRollbackAdapter<CodeRepairState, CodePatchPayload> {
  verifierId = "bounded_unit_test_expression_verifier";
  verifierVersion = "1.0";

  async verify(candidate: TypedCandidate<CodePatchPayload>): Promise<HardVerifierResult> {
    const problem = normalizeCodeProblem(candidate.payload.problem);
    const patch = normalizeCodePatch(candidate.payload.patch);
    const sourceAfter = String(candidate.payload.sourceAfter ?? (candidate.payload as unknown as Record<string, unknown>).source_after ?? "");
    const metadata = {
  ...
  applyCommit(state: CodeRepairState, candidate: TypedCandidate<CodePatchPayload>): CodeRepairState {
  ...
  replay(state: CodeRepairState, receipt: Receipt): CodeRepairState {
    const current = normalizeCodeState(state);
    const payload = (receipt.replayBundle as { candidatePayload: CodePatchPayload }).candidatePayload;
  ...
  rollback(_state: CodeRepairState, receipt: Receipt): CodeRepairState {
    return normalizeCodeState((receipt.rollbackBundle as { preState: CodeRepairState }).preState);
  }

  private reject(kind: CodeResidual["kind"], residual: Omit<CodeResidual, "kind">, metadata: Record<string, unknown>): HardVerifierResult {
...
export class CodeResidualRepairer {
  ...
  acceptedOperators = new Map<string, number>();
  ...
  update(receipt: Receipt): void {
    ...
    const payload = bundle.candidatePayload && typeof bundle.candidatePayload === "object"
      ? bundle.candidatePayload as Partial<CodePatchPayload>
      : {};
  ...
  async propose(candidate: TypedCandidate<CodePatchPayload>, residual: unknown): Promise<TypedCandidate<CodePatchPayload> | null> {
...
export function normalizeCodeTestCase(testInput: CodeTestCase | Record<string, unknown>): CodeTestCase {
  const raw = testInput as Record<string, unknown>;
  const expected = intValue(raw.expected, "expected test output");
  const inputs = normalizeInputs(raw.inputs);
  return { inputs, expected };
}

export function normalizeCodeProblem(problemInput: CodePatchProblem | Record<string, unknown>): CodePatchProblem {
  ...
  const tests = testsInput.map((testCase) => normalizeCodeTestCase(testCase as CodeTestCase | Record<string, unknown>));
...
export function normalizeCodePatch(patchInput: CodePatch | Record<string, unknown>): CodePatch {
...
export function normalizeCodeState(stateInput: CodeRepairState | Record<string, unknown>): CodeRepairState {
  const raw = stateInput as Record<string, unknown>;
  const operator = raw.operator == null ? null : normalizeCodeOperator(raw.operator);
  return {
    problem: normalizeCodeProblem(raw.problem as CodePatchProblem | Record<string, unknown>),
...
export function evaluateOperator(operator: CodeOperator | string, left: number, right: number): number {
...
export function evaluatePatch(problemInput: CodePatchProblem, operator: CodeOperator | string, testInput: CodeTestCase | Record<string, unknown>): number {
...
export function renderSource(problemInput: CodePatchProblem, operatorInput: CodeOperator | string): string {
  const problem = normalizeCodeProblem(problemInput);
  const operator = normalizeCodeOperator(operatorInput);
  const left = problem.leftVar;
  const right = problem.rightVar;
  const expressions: Record<CodeOperator, string> = {
    left,
    right,
    "+": `${left} + ${right}`,
    "-": `${left} - ${right}`,
...
export function diagnoseOperatorRepair(problemInput: CodePatchProblem): CodeResidual["repair"] {
  const problem = normalizeCodeProblem(problemInput);
  const matches: CodeOperator[] = [];
...
export async function makeCodePatchCandidate(
  problemInput: CodePatchProblem,
  operatorInput: CodeOperator | string,
  context = "code-repair",
  cost = 1,
): Promise<TypedCandidate<CodePatchPayload>> {
  const problem = normalizeCodeProblem(problemInput);
  const operator = normalizeCodeOperator(operatorInput);
  const patch: CodePatch = { nodeId: problem.mutableNodeId, operator, baseHash: problem.sourceHash };
...
export async function makeCodeRepairProblem(targetOperatorInput: CodeOperator | string = "+"): Promise<CodePatchProblem> {
  ...
  const problem: CodePatchProblem = {
...
export async function runStaticCodeRepairEpisode(
  problemInput: CodePatchProblem,
  operatorOrder: Array<CodeOperator | string>,
  ledger: Ledger,
  episode: number,
): Promise<CodeRepairEpisodeResult> {
  const problem = normalizeCodeProblem(problemInput);
  const engine = new TransactionEngine(new CodePatchAdapter(), ledger);
  const state: CodeRepairState = { problem, solved: false, operator: null, sourceAfter: null };
...
export async function runRepairCodeEpisode(
  ...
  episode: number,
  initialOperator: CodeOperator | string = INITIAL_CODE_OPERATOR,
): Promise<CodeRepairEpisodeResult> {
  const problem = normalizeCodeProblem(problemInput);
  const engine = new TransactionEngine(new CodePatchAdapter(), ledger);
  const state: CodeRepairState = { problem, solved: false, operator: null, sourceAfter: null };
...
export async function runCodeRepairBenchmark(seed = 59, episodes = 42): Promise<CodeRepairReport> {
  ...
  const staticResults: CodeRepairEpisodeResult[] = [];
  const repairResults: CodeRepairEpisodeResult[] = [];
...
async function candidateShapeError(problem: CodePatchProblem, patch: CodePatch, sourceAfter: string): Promise<string | null> {
...
async function episodeResult(
  calls: number,
  success: boolean,
  engine: TransactionEngine<CodeRepairState, CodePatchPayload>,
  seedState: CodeRepairState,
): Promise<CodeRepairEpisodeResult> {
...
function normalizeCodeOperator(value: unknown): CodeOperator {
  if (typeof value !== "string" || !DEFAULT_CODE_OPERATORS.includes(value as CodeOperator)) {
    throw new RangeError(`unsupported operator: ${String(value)}`);
  }
  return value as CodeOperator;
}

function normalizeInputs(inputsInput: unknown): Record<string, number> {
  ...
  if (Array.isArray(inputsInput)) {
    ...
    entries = Object.entries(inputsInput as Record<string, unknown>);
  }
...
function isCodeResidual(value: unknown): value is CodeResidual {
  if (!value || typeof value !== "object") {
    return false;
  }
  const residual = value as Partial<CodeResidual>;
  return residual.kind === "schema_error" || residual.kind === "test_failure";
}

function codeProblemsEqual(a: CodePatchProblem, b: CodePatchProblem): boolean {
  return JSON.stringify(normalizeCodeProblem(a)) === JSON.stringify(normalizeCodeProblem(b));
}

function codeStatesEqual(a: CodeRepairState, b: CodeRepairState): boolean {
  return JSON.stringify(normalizeCodeState(a)) === JSON.stringify(normalizeCodeState(b));
}

function callsPerSuccess(results: CodeRepairEpisodeResult[]): number {
