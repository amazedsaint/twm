...
export interface TraceProposer<State = unknown> {
  proposerId?: string;
  proposerVersion?: string;
  propose(state: State, budget: Record<string, unknown>): ProposalTrace | Promise<ProposalTrace>;
}

export interface TraceProjector<State = unknown, Payload = unknown> {
  projectorId?: string;
  projectorVersion?: string;
  project(state: State, trace: ProposalTrace): TypedCandidate<Payload> | Promise<TypedCandidate<Payload>>;
}

export interface ReceiptLearner {
  learnerId?: string;
  learnerVersion?: string;
  update(receipt: Receipt): void | Promise<void>;
  snapshotState?(): unknown;
  snapshot?(): unknown;
}

export interface WorldLearnerSnapshot {
  schemaVersion: typeof WORLD_LEARNER_SNAPSHOT_SCHEMA;
  learnerId: string;
  learnerVersion: string;
  updateCount: number;
  sourceReceiptHashes: string[];
  learnerState: unknown;
  learnerStateHash: string;
  snapshotHash: string;
}

export interface WorldLearnerMergeCertificate {
  schemaVersion: typeof WORLD_LEARNER_MERGE_CERTIFICATE_SCHEMA;
  learnerId: string;
  learnerVersion: string;
  mergeStrategy: typeof WORLD_LEARNER_MERGE_STRATEGY;
  mergeBasis: string;
  leftSnapshotHash: string;
  rightSnapshotHash: string;
  mergedSnapshotHash: string;
  baseSnapshotHash: string | null;
  leftUpdateCount: number;
  rightUpdateCount: number;
  mergedUpdateCount: number;
  sharedReceiptCount: number;
  commonPrefixReceiptCount: number;
  conflictCount: number;
  conflictKeys: string[];
  sourceReceiptHashes: string[];
  leftDeltaCertificateHashes: string[];
  rightDeltaCertificateHashes: string[];
  mergedStateHash: string;
  certificateHash: string;
}

export interface WorldLearnerMergeResult {
  mergedSnapshot: WorldLearnerSnapshot;
  certificate: WorldLearnerMergeCertificate;
}

export interface LearnerDeltaOperation {
  op: "set" | "remove";
  path: string[];
  value?: unknown;
}

export interface WorldLearnerDeltaCertificate {
  schemaVersion: typeof WORLD_LEARNER_DELTA_CERTIFICATE_SCHEMA;
  learnerId: string;
  learnerVersion: string;
  ...
  preLearnerStateHash: string;
  postLearnerStateHash: string;
  deltaOpCount: number;
  learnerDelta: LearnerDeltaOperation[];
  learnerDeltaHash: string;
  certificateHash: string;
}

export interface WorldLearnerLineageCertificate {
  schemaVersion: typeof WORLD_LEARNER_LINEAGE_CERTIFICATE_SCHEMA;
  learnerId: string;
  learnerVersion: string;
  initialSnapshotHash: string;
  finalSnapshotHash: string;
  initialUpdateCount: number;
  finalUpdateCount: number;
  updateCertificateCount: number;
  appliedUpdateCount: number;
  sourceReceiptHashes: string[];
  updateCertificateHashes: string[];
  lineageHash: string;
  certificateHash: string;
}

export interface WorldLearnerUpdateCertificate {
  schemaVersion: typeof WORLD_LEARNER_UPDATE_CERTIFICATE_SCHEMA;
  learnerId: string;
  learnerVersion: string;
  sourceReceiptHash: string;
  ...
  preUpdateCount: number;
  postUpdateCount: number;
  preLearnerSnapshotHash: string;
  preLearnerStateHash: string;
  postLearnerStateHash: string;
...
export interface WorldModelStepCertificate {
  schemaVersion: typeof WORLD_MODEL_STEP_CERTIFICATE_SCHEMA;
  proposerId: string;
  proposerVersion: string;
  projectorId: string;
  projectorVersion: string;
  ...
  learnerStateHash: string;
  learnerSnapshotHash: string;
  learnerUpdateCertificateHash: string;
  ledgerHead: string;
  certificateHash: string;
}

export interface WorldModelStepResult<State = unknown, Payload = unknown> {
  state: State;
  committed: boolean;
  receipt: Receipt;
  certificate: WorldModelStepCertificate;
  trace: ProposalTrace;
  candidate: TypedCandidate<Payload>;
...
export class TransactionalWorldModelRuntime<State, Payload> {
  engine: TransactionEngine<State, Payload>;
  proposer: TraceProposer<State>;
  projector: TraceProjector<State, Payload>;
  learner?: ReceiptLearner;
  learnerUpdateCount = 0;
  learnerReceiptHashes: string[] = [];
  ...
  async step(
    state: State,
    options: { budget?: Record<string, unknown>; softScores?: Record<string, number> } = {},
  ): Promise<WorldModelStepResult<State, Payload>> {
...
export async function buildWorldLearnerSnapshot(
  learner: ReceiptLearner | undefined,
  params: {
    updateCount: number;
    sourceReceiptHashes?: string[];
  },
): Promise<WorldLearnerSnapshot> {
  const learnerState = await learnerStateSnapshot(learner);
  const snapshot: WorldLearnerSnapshot = {
...
): Promise<WorldModelStepCertificate> {
  const certificate: WorldModelStepCertificate = {
...
export async function buildWorldLearnerUpdateCertificate(
  receipt: Receipt,
  params: {
    preSnapshot: WorldLearnerSnapshot;
    postSnapshot: WorldLearnerSnapshot;
    updateApplied: boolean;
  },
): Promise<WorldLearnerUpdateCertificate> {
  const certificate: WorldLearnerUpdateCertificate = {
...
export async function buildWorldLearnerDeltaCertificate(
  preSnapshot: WorldLearnerSnapshot,
  postSnapshot: WorldLearnerSnapshot,
  updateCertificate: WorldLearnerUpdateCertificate,
): Promise<WorldLearnerDeltaCertificate> {
  const learnerDelta = learnerStateDelta(preSnapshot.learnerState, postSnapshot.learnerState);
  const certificate: WorldLearnerDeltaCertificate = {
...
export async function buildWorldLearnerLineageCertificate(
  initialSnapshot: WorldLearnerSnapshot,
  finalSnapshot: WorldLearnerSnapshot,
  updateCertificates: WorldLearnerUpdateCertificate[],
): Promise<WorldLearnerLineageCertificate> {
  const certificate: WorldLearnerLineageCertificate = {
...
export async function auditWorldModelStep(
  receipt: Receipt,
  certificate: WorldModelStepCertificate,
  options: { ledgerHead?: string; learnerSnapshot?: WorldLearnerSnapshot; learnerUpdateCertificate?: WorldLearnerUpdateCertificate } = {},
): Promise<boolean> {
  try {
...
export async function validateWorldLearnerSnapshot(snapshot: WorldLearnerSnapshot): Promise<boolean> {
  try {
...
export async function validateWorldLearnerUpdateCertificate(certificate: WorldLearnerUpdateCertificate): Promise<boolean> {
  try {
...
export async function auditWorldLearnerDelta(
  ...
  postSnapshot: WorldLearnerSnapshot,
  updateCertificate: WorldLearnerUpdateCertificate,
  deltaCertificate: WorldLearnerDeltaCertificate,
): Promise<boolean> {
  try {
...
export async function validateWorldLearnerDeltaCertificate(certificate: WorldLearnerDeltaCertificate): Promise<boolean> {
  try {
...
export function applyWorldLearnerDelta(learnerState: unknown, learnerDelta: LearnerDeltaOperation[]): unknown {
  ...
  for (const operation of learnerDelta) {
    if (operation.op === "set") {
      ...
      throw new Error(`unsupported learner delta op: ${(operation as { op?: unknown }).op}`);
    }
  }
  return state;
}
...
export async function validateWorldLearnerLineageCertificate(certificate: WorldLearnerLineageCertificate): Promise<boolean> {
  try {
...
export async function mergeWorldLearnerSnapshots(
  ...
  options: {
    baseSnapshot?: WorldLearnerSnapshot;
    leftDeltaCertificates?: WorldLearnerDeltaCertificate[];
    rightDeltaCertificates?: WorldLearnerDeltaCertificate[];
  } = {},
): Promise<WorldLearnerMergeResult> {
  ...
  const certificate: WorldLearnerMergeCertificate = {
...
export async function validateWorldLearnerMergeCertificate(certificate: WorldLearnerMergeCertificate): Promise<boolean> {
  try {
...
export async function validateWorldModelStepCertificate(certificate: WorldModelStepCertificate): Promise<boolean> {
  try {
...
export async function worldModelStepCertificateHash(certificate: WorldModelStepCertificate): Promise<string> {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function worldLearnerStateHash(learnerState: unknown): Promise<string> {
  return stableHash(learnerState);
}

export async function worldLearnerSnapshotHash(snapshot: WorldLearnerSnapshot): Promise<string> {
  const { snapshotHash: _snapshotHash, ...withoutHash } = snapshot;
  return stableHash(withoutHash);
}

export async function worldLearnerUpdateCertificateHash(certificate: WorldLearnerUpdateCertificate): Promise<string> {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function worldLearnerDeltaHash(learnerDelta: LearnerDeltaOperation[]): Promise<string> {
  return stableHash(learnerDelta.map((operation) => ({ ...operation, path: [...operation.path] })));
}

export async function worldLearnerDeltaCertificateHash(certificate: WorldLearnerDeltaCertificate): Promise<string> {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function worldLearnerLineageHash(certificate: WorldLearnerLineageCertificate): Promise<string> {
  return stableHash({
...
export async function worldLearnerLineageCertificateHash(certificate: WorldLearnerLineageCertificate): Promise<string> {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function worldLearnerMergeCertificateHash(certificate: WorldLearnerMergeCertificate): Promise<string> {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

async function buildMergedWorldLearnerSnapshot(
  base: WorldLearnerSnapshot,
  sourceReceiptHashes: string[],
  learnerState: unknown,
): Promise<WorldLearnerSnapshot> {
  const snapshot: WorldLearnerSnapshot = {
...
function mergeDisjointLearnerState(left: unknown, right: unknown, path = "$"): { value: unknown; conflictKeys: string[] } {
...
async function mergePartialOverlapWithDeltas(
  ...
  leftDeltaCertificates: WorldLearnerDeltaCertificate[],
  rightDeltaCertificates: WorldLearnerDeltaCertificate[],
): Promise<{ mergedState: unknown; mergedReceipts: string[]; commonPrefixReceiptCount: number }> {
...
async function replayWorldLearnerDeltaChain(
  baseSnapshot: WorldLearnerSnapshot,
  finalSnapshot: WorldLearnerSnapshot,
  deltaCertificates: WorldLearnerDeltaCertificate[],
): Promise<{ state: unknown; snapshots: WorldLearnerSnapshot[] }> {
...
function mergeCommonAncestorLearnerState(
  ...
  right: unknown,
  path = "$",
): { value: unknown | typeof MISSING; conflictKeys: string[] } {
...
function learnerStateDelta(pre: unknown, post: unknown, path: string[] = []): LearnerDeltaOperation[] {
  if (stableEqual(pre, post)) {
    return [];
  }
  if (isRecord(pre) && isRecord(post)) {
    const operations: LearnerDeltaOperation[] = [];
...
function validateLearnerDelta(learnerDelta: LearnerDeltaOperation[]): boolean {
...
function setPath(state: unknown, path: string[], value: unknown): unknown {
  if (path.length === 0) {
    return value;
  }
  if (!isRecord(state)) {
...
function removePath(state: unknown, path: string[]): unknown {
  if (path.length === 0) {
...
function setSubset<T>(left: Set<T>, right: Set<T>): boolean {
...
function arrayPrefix(prefix: string[], value: string[]): boolean {
...
function componentId(component: unknown, role: "proposer" | "projector" | "learner"): string {
...
function componentVersion(component: unknown): string {
  const object = component as Record<string, unknown>;
...
async function learnerStateSnapshot(learner: ReceiptLearner | undefined): Promise<unknown> {
  if (!learner) {
    return {};
  }
