...
export interface SdkTransferGuardReport {
  schemaVersion: string;
  routeValid: boolean;
  snapshotValid: boolean;
  baseRouterTopDomain: string;
  guardedRouterTopDomain: string;
  baseRankedDomainIds: string[];
  guardedRankedDomainIds: string[];
  blockedDomainIds: string[];
  sourceBlocked: boolean;
  guardReorderedToTarget: boolean;
...
export async function runSdkTransferGuardBenchmark(): Promise<SdkTransferGuardReport> {
  ...
  const sourceSeed: InventoryState = { stock: { widget: 5 }, reserved: { widget: 0 }, committedOrders: [] };
  const targetSeed: InventoryState = { stock: { widget: 2 }, reserved: { widget: 0 }, committedOrders: [] };
  ...
  const sourceLedger = substrate.domain<InventoryState, InventoryReservationPayload>("source_policy").ledger;
  ...
  const unguardedResidualKind = residual && typeof residual === "object"
    ? String((residual as { kind?: unknown }).kind ?? "")
    : "";
...
function selectionForDomain(domainId: string): string {
  return domainId === "source_policy" ? "quantity-5" : "quantity-2";
}

async function candidateForSelection(state: InventoryState, prefix: string, selection: string) {
