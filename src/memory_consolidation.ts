...
export interface MemoryConsolidationReport {
  ...
  evictedCount: number;
  safeAcceptedCount: number;
  unsafePrefixRejectCount: number;
  terminalRejectCountForgotten: boolean;
  duplicateSafeMerged: boolean;
  safeRank: number;
  unsafeRank: number;
...
export async function runMemoryConsolidationBenchmark(): Promise<MemoryConsolidationReport> {
  const context = "grid-3x3";
  const unsafe: Macro<GridStep> = { macroId: "unsafe-through-wall", steps: ["E", "S", "E", "S"], context, modelVersion: "macro.grid.v1" };
  const safe: Macro<GridStep> = { macroId: "safe-around-wall", steps: ["E", "E", "S", "S"], context, modelVersion: "macro.grid.v1" };
  const safeCopy: Macro<GridStep> = { macroId: "safe-around-wall-copy", steps: ["E", "E", "S", "S"], context, modelVersion: "macro.grid.v1" };
  const terminalMiss: Macro<GridStep> = { macroId: "terminal-miss", steps: ["E"], context, modelVersion: "macro.grid.v1" };
  const staleOob: Macro<GridStep> = { macroId: "stale-oob", steps: ["N"], context, modelVersion: "macro.grid.v1" };
  const ledger = new Ledger();
  const memory = new BoundedMacroMemory<GridStep>(2);
...
async function runMacro(ledger: Ledger, macro: Macro<GridStep>) {
