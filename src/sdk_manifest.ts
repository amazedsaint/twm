...
export interface SdkManifestReport {
  schemaVersion: string;
  domainCount: number;
  manifestValidCount: number;
  manifestAuditCount: number;
  scalarCandidateTypes: string[];
  lifeProjectionSchemas: string[];
  scalarVerifierId: string;
  lifeVerifierId: string;
  scalarReceiptCount: number;
  lifeReceiptCount: number;
  totalReceiptCount: number;
  acceptedCount: number;
  rejectedCount: number;
  hardVerifierCalls: number;
  totalVerifierCost: number;
  manifestHashesStable: boolean;
...
export async function runSdkManifestBenchmark(): Promise<SdkManifestReport> {
