...
import { type Receipt, type TransactionEngine, type TypedCandidate, makeTrace } from "./core.js";
...
export interface BudgetCandidate<CandidatePayload> {
  label: string;
  token: string;
...
export interface BudgetPlan<CandidatePayload> {
  budget: number;
  selected: Array<BudgetCandidate<CandidatePayload>>;
  spent: number;
  expectedUtility: number;
}

export interface BudgetPolicyOutcome<State> {
  state: State;
  committed: boolean;
  committedLabel: string;
  selectedLabels: string[];
  submittedLabels: string[];
  verifierCostSpent: number;
  receipts: Receipt[];
  reason: "commit" | "budget_exhausted";
}

export interface BudgetPolicySnapshot {
  schemaVersion: string;
  rows: BudgetPolicyRow[];
  z: number;
  snapshotHash: string;
}
...
export class ReceiptBudgetPolicy {
  z: number;
  private rows = new Map<string, BudgetPolicyRow>();

  constructor(options: { z?: number } = {}) {
  ...
  update(token: string, receipt: Receipt): BudgetPolicyRow {
  ...
  score(token: string): BudgetPolicyRow {
    const subject = String(token);
  ...
  utility<CandidatePayload>(candidate: BudgetCandidate<CandidatePayload>): number {
    return roundFloat(candidate.reward * this.score(candidate.token).successLowerBound);
  }

  plan<CandidatePayload>(candidates: Array<BudgetCandidate<CandidatePayload>>, budget: number): BudgetPlan<CandidatePayload> {
    ...
    const cells: Array<{ utility: number; subset: Array<BudgetCandidate<CandidatePayload>> } | null> = Array.from(
      { length: budget + 1 },
      () => null,
    );
    ...
    let bestSubset: Array<BudgetCandidate<CandidatePayload>> = [];
    let bestUtility = 0;
    let bestSpent = 0;
  ...
  async submit<State, CandidatePayload>(
    engine: TransactionEngine<State, CandidatePayload>,
    state: State,
    candidates: Array<BudgetCandidate<CandidatePayload>>,
    params: { budget: number; tracePrefix: string; modelVersion?: string },
  ): Promise<BudgetPolicyOutcome<State>> {
  ...
  async snapshot(): Promise<BudgetPolicySnapshot> {
    const pending: BudgetPolicySnapshot = {
  ...
  private scored(row: BudgetPolicyRow): BudgetPolicyRow {
    return {
      ...row,
...
export async function budgetPolicySnapshotHash(snapshot: BudgetPolicySnapshot): Promise<string> {
  const { snapshotHash: _snapshotHash, ...withoutHash } = snapshot;
  return stableHash(withoutHash);
}

export async function validateBudgetPolicySnapshot(snapshot: BudgetPolicySnapshot): Promise<boolean> {
...
function betterSubset<CandidatePayload>(
  subset: Array<BudgetCandidate<CandidatePayload>>,
  utility: number,
  spent: number,
  bestSubset: Array<BudgetCandidate<CandidatePayload>>,
  bestUtility: number,
  bestSpent: number,
): boolean {
...
function labelKey<CandidatePayload>(rows: Array<BudgetCandidate<CandidatePayload>>): string {
...
function validateBudgetCandidate<CandidatePayload>(candidate: BudgetCandidate<CandidatePayload>): void {
  if (!candidate.label) {
