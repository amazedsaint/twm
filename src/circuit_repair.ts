...
export type TruthAssignment = number[];
export type TruthOutput = number[];
...
export interface CircuitRepairProblem {
  templateNetlist: BooleanNetlist;
  targetTable: TruthRow[];
  mutableGateId: string;
  allowedOps: number[];
}

export interface CircuitRepairState {
  problem: CircuitRepairProblem;
  solved: boolean;
  netlist?: BooleanNetlist | null;
}

export interface CircuitCandidatePayload {
  context: string;
  problem: CircuitRepairProblem;
...
export interface CircuitResidual {
  kind: "schema_error" | "truth_table_mismatch";
  message?: string;
  row?: number;
  assignment?: TruthAssignment;
  expected?: TruthOutput;
  actual?: TruthOutput;
  repair?: { gateId: string; opMask: number; opName: string } | null;
}

export interface CircuitEpisodeResult {
  calls: number;
  success: boolean;
  auditOk: boolean;
...
export interface CircuitRepairReport {
  episodes: number;
  opCount: number;
...
export class BooleanCircuitAdapter implements ReplayRollbackAdapter<CircuitRepairState, CircuitCandidatePayload> {
  verifierId = "boolean_circuit_truth_table_verifier";
  verifierVersion = "1.0";

  verify(candidate: TypedCandidate<CircuitCandidatePayload>): HardVerifierResult {
  ...
  applyCommit(state: CircuitRepairState, candidate: TypedCandidate<CircuitCandidatePayload>): CircuitRepairState {
  ...
  replay(state: CircuitRepairState, receipt: Receipt): CircuitRepairState {
    const current = normalizeCircuitState(state);
    const payload = (receipt.replayBundle as { candidatePayload: CircuitCandidatePayload }).candidatePayload;
  ...
  rollback(_state: CircuitRepairState, receipt: Receipt): CircuitRepairState {
    return normalizeCircuitState((receipt.rollbackBundle as { preState: CircuitRepairState }).preState);
  }

  private reject(kind: CircuitResidual["kind"], residual: Omit<CircuitResidual, "kind">, metadata: Record<string, unknown>): HardVerifierResult {
...
export class CircuitResidualRepairer {
  ...
  acceptedOps = new Map<string, number>();
  ...
  update(receipt: Receipt): void {
    ...
    const payload = bundle.candidatePayload && typeof bundle.candidatePayload === "object"
      ? bundle.candidatePayload as Partial<CircuitCandidatePayload>
      : {};
  ...
  async propose(candidate: TypedCandidate<CircuitCandidatePayload>, residual: unknown): Promise<TypedCandidate<CircuitCandidatePayload> | null> {
...
export function evalOpMask(opMaskInput: number, leftInput: number, rightInput: number): number {
...
export function normalizeGate(gateInput: CircuitGate | Record<string, unknown>): CircuitGate {
  const raw = gateInput as Record<string, unknown>;
...
export function normalizeNetlist(netlistInput: BooleanNetlist | Record<string, unknown>): BooleanNetlist {
  const raw = netlistInput as Record<string, unknown>;
  ...
  const gates = raw.gates.map((gate) => normalizeGate(gate as CircuitGate | Record<string, unknown>));
...
export function normalizeTruthRow(rowInput: TruthRow | Record<string, unknown>): TruthRow {
  const raw = rowInput as Record<string, unknown>;
...
export function normalizeCircuitProblem(problemInput: CircuitRepairProblem | Record<string, unknown>): CircuitRepairProblem {
  const raw = problemInput as Record<string, unknown>;
  const template = normalizeNetlist((raw.templateNetlist ?? raw.template_netlist) as BooleanNetlist | Record<string, unknown>);
  ...
  const targetTable = targetTableInput.map((row) => normalizeTruthRow(row as TruthRow | Record<string, unknown>));
...
export function normalizeCircuitState(stateInput: CircuitRepairState): CircuitRepairState {
  return {
...
export function evaluateNetlist(netlistInput: BooleanNetlist, assignmentInput: TruthAssignment): TruthOutput {
...
export function truthTable(netlistInput: BooleanNetlist): TruthRow[] {
  const netlist = normalizeNetlist(netlistInput);
  return assignments(netlist.inputs.length).map((assignment) => ({ assignment, outputs: evaluateNetlist(netlist, assignment) }));
}

export function replaceGateOp(netlistInput: BooleanNetlist, gateId: string, opMaskInput: number): BooleanNetlist {
  const netlist = normalizeNetlist(netlistInput);
  const opMask = normalizeOpMask(opMaskInput);
  let changed = false;
...
export function diagnoseMutableGate(problemInput: CircuitRepairProblem, netlistInput: BooleanNetlist): { gateId: string; opMask: number; opName: string } | null {
...
export async function makeCircuitCandidate(
  problemInput: CircuitRepairProblem,
  opMaskInput: number,
  context = "circuit",
  cost = 1,
): Promise<TypedCandidate<CircuitCandidatePayload>> {
  const problem = normalizeCircuitProblem(problemInput);
  const opMask = normalizeOpMask(opMaskInput);
...
export function makeCircuitRepairProblem(targetOpMaskInput = 6): CircuitRepairProblem {
  const targetOpMask = normalizeOpMask(targetOpMaskInput);
  const template: BooleanNetlist = {
    inputs: ["x0", "x1", "x2"],
    gates: [
...
export async function runStaticCircuitEpisode(
  ...
  ledger: Ledger,
  episode: number,
): Promise<CircuitEpisodeResult> {
  const problem = normalizeCircuitProblem(problemInput);
  const engine = new TransactionEngine(new BooleanCircuitAdapter(), ledger);
  const state: CircuitRepairState = { problem, solved: false, netlist: null };
...
export async function runCircuitRepairBenchmark(seed = 47, episodes = 45): Promise<CircuitRepairReport> {
  if (!Number.isInteger(episodes) || episodes <= 0) {
    throw new RangeError("episodes must be positive");
  }
  const opMasks = shuffledOps(seed);
  const staticResults: CircuitEpisodeResult[] = [];
  const repairResults: CircuitEpisodeResult[] = [];
...
function candidateShapeError(problem: CircuitRepairProblem, netlist: BooleanNetlist): string | null {
...
async function episodeResult(
  calls: number,
  success: boolean,
  engine: TransactionEngine<CircuitRepairState, CircuitCandidatePayload>,
  seedState: CircuitRepairState,
): Promise<CircuitEpisodeResult> {
...
function normalizeOpMask(value: number): number {
...
function assignments(width: number): TruthAssignment[] {
...
function bitsEqualStrings(a: readonly string[], b: readonly string[]): boolean {
  return a.length === b.length && a.every((value, idx) => value === b[idx]);
}

function circuitProblemsEqual(a: CircuitRepairProblem, b: CircuitRepairProblem): boolean {
  return JSON.stringify(normalizeCircuitProblem(a)) === JSON.stringify(normalizeCircuitProblem(b));
}

function circuitStatesEqual(a: CircuitRepairState, b: CircuitRepairState): boolean {
  return JSON.stringify(normalizeCircuitState(a)) === JSON.stringify(normalizeCircuitState(b));
}

function isCircuitResidual(value: unknown): value is CircuitResidual {
  if (!value || typeof value !== "object") {
    return false;
  }
  const residual = value as Partial<CircuitResidual>;
  return residual.kind === "schema_error" || residual.kind === "truth_table_mismatch";
}

function callsPerSuccess(results: CircuitEpisodeResult[]): number {
...
function shuffledOps(seed: number): number[] {
