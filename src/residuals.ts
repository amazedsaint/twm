...
const residualCategories = new Set<ResidualCategory>([
  "none",
  "constraint",
  "coverage",
  "resource",
...
const kindCategory: Record<string, ResidualCategory> = {
  none: "none",
  schema_error: "schema",
...
export class ResidualTaxonomyMemory {
  ...
  hintCounts = new Map<string, Map<string, number>>();

  async update(signal: ResidualSignal): Promise<void> {
  ...
  topRepairHint(kind: string): string | null {
...
export async function residualSignalFromReceipt(receipt: Receipt, options: { sourceDomain?: string } = {}): Promise<ResidualSignal> {
...
export async function residualSignalHash(signal: ResidualSignal): Promise<string> {
  const { signalHash: _signalHash, ...withoutHash } = signal;
  return stableHash(withoutHash);
}

export async function residualLearningHash(signal: ResidualSignal): Promise<string> {
  return stableHash({
...
export async function validateResidualSignal(signal: ResidualSignal): Promise<boolean> {
...
function normalizeMapping(mapping: Record<string, unknown>): Record<string, unknown> {
...
function extractFields(residual: Record<string, unknown>): string[] {
  const fields: string[] = [];
...
function extractRepairHints(residual: Record<string, unknown>): string[] {
  const hints: string[] = [];
...
function flattenRepairHint(key: string, value: unknown): string[] {
  if (isRecord(value)) {
