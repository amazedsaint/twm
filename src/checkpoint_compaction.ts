...
export interface CheckpointCompactionReport {
  schemaVersion: string;
  receiptCount: number;
  committedCount: number;
  checkpointReceiptCount: number;
  checkpointCommittedCount: number;
  suffixReceiptCount: number;
  fullReplayCommits: number;
  checkpointReplayCommits: number;
  replayCallsSaved: number;
  finalStateHashEqual: boolean;
  finalHeadEqual: boolean;
  checkpointValid: boolean;
  compactedAudit: boolean;
  tamperDetected: boolean;
  staleSuffixRejected: boolean;
  originalLedgerAudit: boolean;
  invalidCommitCount: number;
}

export async function runCheckpointCompactionBenchmark(): Promise<CheckpointCompactionReport> {
...
async function makeCheckpointLedger(): Promise<[InventoryState, InventoryState, TransactionEngine<InventoryState, InventoryReservationPayload>]> {
  const seedState: InventoryState = { stock: { widget: 10 }, reserved: { widget: 0 }, committedOrders: [] };
  let state = seedState;
