...
export interface CrossDomainTransferReport {
  schemaVersion: string;
  certificateValid: boolean;
  positiveTransferClaimSupported: boolean;
  positiveTransferClaimRejected: boolean;
  claimId: string;
  sourceDomainCount: number;
  targetDomainCount: number;
  sourceReceiptCount: number;
  targetEvaluationReceiptCount: number;
...
export async function runCrossDomainTransferAudit(): Promise<CrossDomainTransferReport> {
  ...
  const transferResidualKind = transferResidual && typeof transferResidual === "object"
    ? String((transferResidual as { kind?: unknown }).kind ?? "")
    : "";
