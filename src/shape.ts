...
export interface ShapeCandidatePayload {
  context: string;
  action: number;
  guess: number;
  defect: number;
...
export interface ShapeEpisodeResult {
  calls: number;
  success: boolean;
}

export interface ShapeReport {
  lowRandomCallsPerSuccess: number;
  lowReceiptMemoryCallsPerSuccess: number;
  lowHdcMemoryCallsPerSuccess: number;
  highRandomCallsPerSuccess: number;
  highReceiptMemoryCallsPerSuccess: number;
  highHdcMemoryCallsPerSuccess: number;
  lowReceiptGain: number;
  lowHdcGain: number;
  highReceiptGain: number;
  highHdcGain: number;
  lowPreflightR90: number;
  highPreflightR90: number;
  lowPreflightFitsBudget: boolean;
  highPreflightFitsBudget: boolean;
  lowPreflightEnergyAtBudget: number;
  highPreflightEnergyAtBudget: number;
  ledgerAudit: boolean;
  invalidCommitCount: number;
}

type ShapeOrderer = (family: string, episode: number, labels: number[]) => Promise<number[]> | number[];

export class ShapeGuessAdapter implements ReplayRollbackAdapter<ShapeState, ShapeCandidatePayload> {
  verifierId = "shape_oracle";
  verifierVersion = "1.0";

  verify(candidate: TypedCandidate<ShapeCandidatePayload>): HardVerifierResult {
  ...
  applyCommit(state: ShapeState, candidate: TypedCandidate<ShapeCandidatePayload>): ShapeState {
    const { guess, defect } = candidate.payload;
    return { ...state, solved: true, guess, defect };
  }

  replay(state: ShapeState, receipt: Receipt): ShapeState {
    const payload = (receipt.replayBundle as { candidatePayload: ShapeCandidatePayload }).candidatePayload;
    return { ...state, solved: true, guess: payload.guess, defect: payload.defect };
  }

  rollback(_state: ShapeState, receipt: Receipt): ShapeState {
    return (receipt.rollbackBundle as { preState: ShapeState }).preState;
  }
}

export async function runShapeConditionality(seed = 7, episodes = 96, labelCount = 24): Promise<ShapeReport> {
  ...
  const staticOrder: ShapeOrderer = () => randomOrder;
...
async function runOrder(
  ...
  engine: TransactionEngine<ShapeState, ShapeCandidatePayload>,
  learners: { ranker?: ReceiptRanker; memory?: HyperdimensionalMemory } = {},
  ...
  for (let episode = 0; episode < defects.length; episode += 1) {
    ...
    const state: ShapeState = { family, episode, solved: false };
    let calls = 0;
    let success = false;
    ...
    for (const guess of order) {
      ...
      const candidate = makeCandidate<ShapeCandidatePayload>(
...
function shapeAction(receipt: Receipt): number | null {
  const replayBundle = receipt.replayBundle as { candidatePayload?: Partial<ShapeCandidatePayload> };
