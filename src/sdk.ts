...
import {
  type TransferEvaluationCertificate,
  type TransferGuardDecision,
  type TransferGuardEntry,
  type TransferGuardSnapshot,
  TransferGuardMemory,
} from "./transfer.js";
...
export interface DomainRuntime<State = unknown, Payload = unknown> {
  domainId: string;
  adapter: ReplayRollbackAdapter<State, Payload>;
  ledger: Ledger;
  hardVerifierCalls: number;
}

export interface SdkTransactionResult<State = unknown> {
  domainId: string;
  outcome: TransactionOutcome<State>;
  hardVerifierCalls: number;
  committed: boolean;
  receipt: Receipt;
}
...
export interface DomainAudit {
  domainId: string;
  ledgerAudit: boolean;
  replayMatchesReceipts: boolean;
  rollbackMatchesSeed: boolean;
  invalidCommitCount: number;
  ok: boolean;
}

export interface DomainManifestCertificate {
  schemaVersion: typeof SDK_DOMAIN_MANIFEST_SCHEMA;
  domainId: string;
  adapterType: string;
  verifierId: string;
  verifierVersion: string;
  candidateTypeNames: string[];
  projectionSchemaVersions: string[];
...
export interface TransferGuardedDomainRoute {
  schemaVersion: typeof TRANSFER_GUARDED_DOMAIN_ROUTE_SCHEMA;
  context: string;
  sourceDomains: string[];
  targetDomain: string;
  inputDomainIds: string[];
  baseRankedDomainIds: string[];
  rankedDomainIds: string[];
  blockedDomainIds: string[];
  decisionReason: string;
  decisionAdmitted: boolean;
  decisionHash: string;
  routeHash: string;
  topDomainId: string;
...
export interface VerifierCostStats {
  accepted: number;
  rejected: number;
  abstained: number;
  verifierCost: number;
  calls: number;
  successPerCostNumerator: number;
  successPerCostDenominator: number;
  successPerCost: number;
}

export interface DomainRouter {
  update(domainId: string, context: string, receipt: Receipt): void;
  rank(context: string, domainIds: string[]): string[];
  counts(context: string, domainId: string): { accepted: number; rejected: number };
}
...
export class CostAwareReceiptDomainRouter implements DomainRouter {
  defaultVerifierCost: number;
  rows = new Map<string, Map<string, MutableVerifierCostStats>>();
  invalidCostMetadata = new Map<string, Map<string, number>>();
  ...
  stats(context: string, domainId: string): VerifierCostStats {
    const row = this.mutableStats(context, domainId);
  ...
  private mutableStats(context: string, domainId: string): MutableVerifierCostStats {
...
export class TransferGuardedDomainRouter implements DomainRouter {
  baseRouter: DomainRouter;
  transferGuard: TransferGuardMemory;

  constructor(baseRouter: DomainRouter = new ReceiptDomainRouter(), transferGuard: TransferGuardMemory = new TransferGuardMemory()) {
  ...
  async updateTransferCertificate(certificate: TransferEvaluationCertificate): Promise<TransferGuardEntry[]> {
    return this.transferGuard.update(certificate);
  }

  async guardSnapshot(): Promise<TransferGuardSnapshot> {
    return this.transferGuard.snapshot();
  }

  async decideTransfer(sourceDomains: string[], targetDomain: string): Promise<TransferGuardDecision> {
  ...
  async rankWithTransferGuard(
    ...
    sourceDomains: string[],
    targetDomain: string,
  ): Promise<TransferGuardedDomainRoute> {
    ...
    const pending: Omit<TransferGuardedDomainRoute, "routeHash" | "topDomainId" | "sourceBlocked"> & {
      routeHash: string;
      topDomainId: string;
      sourceBlocked: boolean;
    } = {
...
export async function buildDomainManifest(runtime: DomainRuntime): Promise<DomainManifestCertificate> {
  ...
  const certificate: DomainManifestCertificate = {
...
export async function auditDomainManifest(runtime: DomainRuntime, certificate: DomainManifestCertificate): Promise<boolean> {
  try {
...
export async function validateDomainManifest(certificate: DomainManifestCertificate): Promise<boolean> {
  try {
...
export async function domainManifestHash(certificate: DomainManifestCertificate): Promise<string> {
  const { manifestHash: _manifestHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function transferGuardedDomainRouteHash(route: TransferGuardedDomainRoute | Record<string, unknown>): Promise<string> {
  const { routeHash: _routeHash, ...withoutHash } = route as Record<string, unknown>;
  return stableHash(withoutHash);
}

export async function validateTransferGuardedDomainRoute(route: TransferGuardedDomainRoute): Promise<boolean> {
  try {
...
export class ProgrammableSubstrate {
  router: DomainRouter;
  domains = new Map<string, DomainRuntime<unknown, unknown>>();

  constructor(router: DomainRouter = new ReceiptDomainRouter()) {
    this.router = router;
  }
  ...
  register<State, Payload>(
    domainId: string,
    adapter: ReplayRollbackAdapter<State, Payload>,
  ): DomainRuntime<State, Payload> {
    if (this.domains.has(domainId)) {
      throw new Error(`domain already registered: ${domainId}`);
    }
    const runtime: DomainRuntime<State, Payload> = {
      domainId,
      adapter,
      ledger: new Ledger(),
      hardVerifierCalls: 0,
    };
    this.domains.set(domainId, runtime as DomainRuntime<unknown, unknown>);
    return runtime;
  }
  ...
  async submit<State, Payload>(
    ...
    options: { context?: string; softScores?: Record<string, number> } = {},
  ): Promise<SdkTransactionResult<State>> {
    const domain = this.domain<State, Payload>(domainId);
  ...
  rankDomains(context: string, domainIds?: string[]): string[] {
    return this.router.rank(context, domainIds ?? Array.from(this.domains.keys()));
  }

  async auditDomain<State>(domainId: string, seedState: State): Promise<DomainAudit> {
    const domain = this.domain<State, unknown>(domainId);
    const engine = new TransactionEngine(domain.adapter, domain.ledger);
  ...
  async domainManifest(domainId: string): Promise<DomainManifestCertificate> {
    return buildDomainManifest(this.domain(domainId));
  }

  async auditDomainManifest(domainId: string, certificate: DomainManifestCertificate): Promise<boolean> {
    return auditDomainManifest(this.domain(domainId), certificate);
  }

  invalidCommitCount(domainIds?: string[]): number {
  ...
  domain<State = unknown, Payload = unknown>(domainId: string): DomainRuntime<State, Payload> {
    const domain = this.domains.get(domainId);
    if (!domain) {
      throw new Error(`unknown domain: ${domainId}`);
    }
    return domain as DomainRuntime<State, Payload>;
  }
}

function increment(table: Map<string, Map<string, number>>, context: string, domainId: string): void {
...
function getCount(table: Map<string, Map<string, number>>, context: string, domainId: string): number {
  return table.get(context)?.get(domainId) ?? 0;
}

interface MutableVerifierCostStats {
  accepted: number;
  rejected: number;
  abstained: number;
  verifierCost: number;
...
export function verifierCostUnits(receipt: Receipt, defaultVerifierCost = 1): { cost: number; metadataOk: boolean } {
...
function receiptCandidateType(receipt: Receipt): string {
...
function uniqueOrdered(values: unknown[]): string[] {
  const rows: string[] = [];
...
function sortedUniqueNonempty(values: string[]): boolean {
...
function sortedUniqueNonemptyWhenReceipts(values: string[], receiptCount: number): boolean {
...
function compareCostAwareRank(a: VerifierCostStats, b: VerifierCostStats, aIdx: number, bIdx: number): number {
