...
export const NORMAL_VALENCE: Record<string, number> = { C: 4, N: 3, O: 2, F: 1, Cl: 1 };
export const DEFAULT_ELEMENTS = ["C", "N", "O", "F", "Cl"] as const;
export const DEFAULT_BOND_ORDERS = [1, 2, 3] as const;
...
export interface MoleculeRepairProblem {
  templateGraph: MoleculeGraph;
  targetFormula: Record<string, number>;
  mutableAtomId: string;
  mutableBondId: string;
  allowedElements: string[];
  allowedBondOrders: number[];
}

export interface MoleculeRepairState {
  problem: MoleculeRepairProblem;
  solved: boolean;
  graph?: MoleculeGraph | null;
}

export interface MoleculeCandidatePayload {
  context: string;
  problem: MoleculeRepairProblem;
  graph: MoleculeGraph;
  mutableAtomId: string;
...
export interface MoleculeResidual {
  kind: "schema_error" | "valence_exceeded" | "formula_mismatch";
  message?: string;
  violations?: Array<{ atomId: string; element: string; explicit: number; max: number }>;
  expected?: Record<string, number>;
  actual?: Record<string, number>;
  repair?: { atomId: string; bondId: string; element: string; bondOrder: number; formula: Record<string, number> } | null;
}

export interface MoleculeEpisodeResult {
  calls: number;
  success: boolean;
  auditOk: boolean;
...
export interface MoleculeRepairReport {
  episodes: number;
...
export class MoleculeGraphAdapter implements ReplayRollbackAdapter<MoleculeRepairState, MoleculeCandidatePayload> {
  verifierId = "organic_subset_valence_formula_verifier";
  verifierVersion = "1.0";

  verify(candidate: TypedCandidate<MoleculeCandidatePayload>): HardVerifierResult {
  ...
  applyCommit(state: MoleculeRepairState, candidate: TypedCandidate<MoleculeCandidatePayload>): MoleculeRepairState {
  ...
  replay(state: MoleculeRepairState, receipt: Receipt): MoleculeRepairState {
    const current = normalizeMoleculeState(state);
    const payload = (receipt.replayBundle as { candidatePayload: MoleculeCandidatePayload }).candidatePayload;
  ...
  rollback(_state: MoleculeRepairState, receipt: Receipt): MoleculeRepairState {
    return normalizeMoleculeState((receipt.rollbackBundle as { preState: MoleculeRepairState }).preState);
  }

  private reject(kind: MoleculeResidual["kind"], residual: Omit<MoleculeResidual, "kind">, metadata: Record<string, unknown>): HardVerifierResult {
...
export class MoleculeResidualRepairer {
  ...
  acceptedEdits = new Map<string, number>();
  ...
  update(receipt: Receipt): void {
    ...
    const payload = bundle.candidatePayload && typeof bundle.candidatePayload === "object"
      ? bundle.candidatePayload as Partial<MoleculeCandidatePayload>
      : {};
  ...
  async propose(candidate: TypedCandidate<MoleculeCandidatePayload>, residual: unknown): Promise<TypedCandidate<MoleculeCandidatePayload> | null> {
...
export function normalizeMoleculeAtom(atomInput: MoleculeAtom | Record<string, unknown>): MoleculeAtom {
  const raw = atomInput as Record<string, unknown>;
...
export function normalizeMoleculeBond(bondInput: MoleculeBond | Record<string, unknown>): MoleculeBond {
  const raw = bondInput as Record<string, unknown>;
...
export function normalizeMoleculeGraph(graphInput: MoleculeGraph | Record<string, unknown>): MoleculeGraph {
  const raw = graphInput as Record<string, unknown>;
  if (!Array.isArray(raw.atoms) || !Array.isArray(raw.bonds)) {
    throw new RangeError("molecule graph requires atoms and bonds arrays");
  }
  const atoms = raw.atoms.map((atom) => normalizeMoleculeAtom(atom as MoleculeAtom | Record<string, unknown>));
  ...
  const bonds = raw.bonds.map((bond) => normalizeMoleculeBond(bond as MoleculeBond | Record<string, unknown>));
...
export function normalizeFormula(formulaInput: Record<string, unknown>): Record<string, number> {
...
export function normalizeMoleculeProblem(problemInput: MoleculeRepairProblem | Record<string, unknown>): MoleculeRepairProblem {
  const raw = problemInput as Record<string, unknown>;
  const template = normalizeMoleculeGraph((raw.templateGraph ?? raw.template_graph) as MoleculeGraph | Record<string, unknown>);
  const targetFormula = normalizeFormula((raw.targetFormula ?? raw.target_formula) as Record<string, unknown>);
...
export function normalizeMoleculeState(stateInput: MoleculeRepairState): MoleculeRepairState {
  return {
...
export function explicitValences(graphInput: MoleculeGraph): Record<string, number> {
  const graph = normalizeMoleculeGraph(graphInput);
  const valences: Record<string, number> = Object.fromEntries(graph.atoms.map((atom) => [atom.atomId, 0]));
...
export function valenceViolations(graphInput: MoleculeGraph): Array<{ atomId: string; element: string; explicit: number; max: number }> {
  const graph = normalizeMoleculeGraph(graphInput);
  const valences = explicitValences(graph);
  const violations: Array<{ atomId: string; element: string; explicit: number; max: number }> = [];
...
export function molecularFormula(graphInput: MoleculeGraph): Record<string, number> {
  const graph = normalizeMoleculeGraph(graphInput);
  const violations = valenceViolations(graph);
  if (violations.length > 0) {
...
export function replaceMoleculeEdit(
  graphInput: MoleculeGraph,
  atomId: string,
  element: string,
  bondId: string,
  bondOrderInput: number,
): MoleculeGraph {
  const graph = normalizeMoleculeGraph(graphInput);
...
export function diagnoseMoleculeEdit(problemInput: MoleculeRepairProblem, graphInput: MoleculeGraph): MoleculeResidual["repair"] {
  const problem = normalizeMoleculeProblem(problemInput);
  normalizeMoleculeGraph(graphInput);
  const matches: Array<{ element: string; bondOrder: number; formula: Record<string, number> }> = [];
...
export async function makeMoleculeCandidate(
  problemInput: MoleculeRepairProblem,
  element: string,
  bondOrderInput: number,
  context = "molecule",
  cost = 1,
): Promise<TypedCandidate<MoleculeCandidatePayload>> {
  const problem = normalizeMoleculeProblem(problemInput);
  const bondOrder = normalizeBondOrder(bondOrderInput);
  const graph = replaceMoleculeEdit(problem.templateGraph, problem.mutableAtomId, element, problem.mutableBondId, bondOrder);
...
export function makeMoleculeRepairProblem(targetElement = "O", targetBondOrderInput = 1): MoleculeRepairProblem {
  const targetBondOrder = normalizeBondOrder(targetBondOrderInput);
  const template: MoleculeGraph = {
    atoms: [
      { atomId: "a0", element: "C" },
      { atomId: "a1", element: "C" },
      { atomId: "a2", element: "C" },
...
export async function runStaticMoleculeEpisode(
  problemInput: MoleculeRepairProblem,
  editOrder: Array<[string, number]>,
  ledger: Ledger,
  episode: number,
): Promise<MoleculeEpisodeResult> {
  const problem = normalizeMoleculeProblem(problemInput);
  const engine = new TransactionEngine(new MoleculeGraphAdapter(), ledger);
  const state: MoleculeRepairState = { problem, solved: false, graph: null };
...
export async function runMoleculeRepairBenchmark(seed = 53, episodes = 42): Promise<MoleculeRepairReport> {
  if (!Number.isInteger(episodes) || episodes <= 0) {
    throw new RangeError("episodes must be positive");
  }
  const editSpace: Array<[string, number]> = [];
  ...
  const staticResults: MoleculeEpisodeResult[] = [];
  const repairResults: MoleculeEpisodeResult[] = [];
...
function candidateShapeError(problem: MoleculeRepairProblem, graph: MoleculeGraph): string | null {
...
async function episodeResult(
  calls: number,
  success: boolean,
  engine: TransactionEngine<MoleculeRepairState, MoleculeCandidatePayload>,
  seedState: MoleculeRepairState,
): Promise<MoleculeEpisodeResult> {
...
function normalizeBondOrder(value: number): number {
  if (!Number.isInteger(value) || !DEFAULT_BOND_ORDERS.includes(value as 1 | 2 | 3)) {
    throw new RangeError("bond order must be 1, 2, or 3");
  }
  return value;
}

function formulasEqual(a: Record<string, number>, b: Record<string, number>): boolean {
  return JSON.stringify(normalizeFormula(a)) === JSON.stringify(normalizeFormula(b));
}

function moleculeProblemsEqual(a: MoleculeRepairProblem, b: MoleculeRepairProblem): boolean {
  return JSON.stringify(normalizeMoleculeProblem(a)) === JSON.stringify(normalizeMoleculeProblem(b));
}

function moleculeStatesEqual(a: MoleculeRepairState, b: MoleculeRepairState): boolean {
...
function isMoleculeResidual(value: unknown): value is MoleculeResidual {
  if (!value || typeof value !== "object") {
    return false;
  }
  const residual = value as Partial<MoleculeResidual>;
  return residual.kind === "schema_error" || residual.kind === "valence_exceeded" || residual.kind === "formula_mismatch";
}

function callsPerSuccess(results: MoleculeEpisodeResult[]): number {
