...
export const TRANSFER_EVALUATION_CONCLUSIONS = ["positive_transfer", "negative_transfer", "neutral"] as const;
export const TRANSFER_GUARD_SNAPSHOT_SCHEMA = "trwm.transfer_guard_snapshot.v1";

export type TransferEvaluationConclusion = typeof TRANSFER_EVALUATION_CONCLUSIONS[number];

export interface TransferEvaluationCertificate {
  schemaVersion: typeof TRANSFER_EVALUATION_CERTIFICATE_SCHEMA;
  claimId: string;
  learnerId: string;
  ...
  targetEvaluationReceiptHashes: string[];
  baselineName: string;
  transferName: string;
  baselineSuccessCount: number;
  transferSuccessCount: number;
  baselineVerifierCalls: number;
  transferVerifierCalls: number;
  sameCaseBaseline: boolean;
  hardCommitOnly: boolean;
  invalidCommitCount: number;
  ledgerAudit: boolean;
  replayRollbackRate: number;
  sourceTargetDomainDisjoint: boolean;
  sourceTargetReceiptDisjoint: boolean;
  successDelta: number;
  verifierCallDelta: number;
  conclusion: TransferEvaluationConclusion;
  metrics: Record<string, unknown>;
  certificateHash: string;
}

export interface TransferGuardEntry {
  sourceDomains: string[];
  targetDomain: string;
  conclusion: TransferEvaluationConclusion;
  certificateHash: string;
...
export interface TransferGuardDecision {
  sourceDomains: string[];
  targetDomain: string;
  admitted: boolean;
  reason: string;
...
export interface TransferGuardSnapshot {
  schemaVersion: typeof TRANSFER_GUARD_SNAPSHOT_SCHEMA;
  entries: TransferGuardEntry[];
  snapshotHash: string;
}

export class TransferGuardMemory {
  private entries = new Map<string, TransferGuardEntry>();

  async update(certificate: TransferEvaluationCertificate): Promise<TransferGuardEntry[]> {
  ...
  async updateValidated(certificate: TransferEvaluationCertificate): Promise<TransferGuardEntry[]> {
    return this.update(certificate);
  }

  private applyUpdate(certificate: TransferEvaluationCertificate): TransferGuardEntry[] {
    const rows: TransferGuardEntry[] = [];
    for (const targetDomain of certificate.targetDomains) {
      const entry: TransferGuardEntry = {
  ...
  async decide(sourceDomains: string[], targetDomain: string): Promise<TransferGuardDecision> {
  ...
  async snapshot(): Promise<TransferGuardSnapshot> {
    const pending: TransferGuardSnapshot = {
...
export async function buildTransferEvaluationCertificate(params: {
  ...
  replayRollbackRate: number;
  metrics?: Record<string, unknown>;
}): Promise<TransferEvaluationCertificate> {
  ...
  const pending: TransferEvaluationCertificate = {
...
export async function buildTransferGuardDecision(params: {
  ...
  conclusion?: "" | TransferEvaluationConclusion;
  certificateHash?: string;
}): Promise<TransferGuardDecision> {
  const pending: TransferGuardDecision = {
...
export async function transferEvaluationCertificateHash(certificate: TransferEvaluationCertificate): Promise<string> {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function transferGuardSnapshotHash(snapshot: TransferGuardSnapshot): Promise<string> {
  const { snapshotHash: _snapshotHash, ...withoutHash } = snapshot;
  return stableHash(withoutHash);
}

export async function transferGuardDecisionHash(decision: TransferGuardDecision): Promise<string> {
  const { decisionHash: _decisionHash, ...withoutHash } = decision;
  return stableHash(withoutHash);
}

export async function validateTransferEvaluationCertificate(certificate: TransferEvaluationCertificate): Promise<boolean> {
  try {
...
export async function validateTransferGuardSnapshot(snapshot: TransferGuardSnapshot): Promise<boolean> {
  try {
...
export async function validateTransferGuardDecision(decision: TransferGuardDecision): Promise<boolean> {
  try {
...
export async function transferEvaluationSupportsPositiveClaim(certificate: TransferEvaluationCertificate): Promise<boolean> {
...
export async function transferEvaluationRejectsPositiveClaim(certificate: TransferEvaluationCertificate): Promise<boolean> {
  return await validateTransferEvaluationCertificate(certificate)
...
function transferConclusion(successDelta: number, verifierCallDelta: number): TransferEvaluationConclusion {
...
function guardKey(sourceDomains: string[], targetDomain: string): string {
  return `${sourceDomains.join("\u0000")}\u0001${targetDomain}`;
}

function compareEntry(left: TransferGuardEntry, right: TransferGuardEntry): number {
...
function moreConservative(candidate: TransferGuardEntry, existing: TransferGuardEntry): boolean {
...
function conclusionPriority(conclusion: string): number {
...
function uniqueHashes(values: string[]): boolean {
