...
import { type ResidualRepairOption, ResidualTopKSubmitter } from "./topk.js";

export const RESIDUAL_TOPK_ORDER = [8, 7, 5, 4] as const;
export const RESIDUAL_TOPK_LIMIT = 2;

export interface ResidualTopKReport {
  trainingResidualKind: string;
  learnedRepairHint: string;
  candidateCount: number;
  topK: number;
  unrankedSubmitted: string[];
  unrankedCommitted: boolean;
  unrankedVerifierCalls: number;
  residualRankedSubmitted: string[];
  residualRankedCommitted: boolean;
  residualRankedCommittedLabel: string;
  residualRankedVerifierCalls: number;
  callsToCommitGain: number;
  ledgerAudit: boolean;
  replayRollbackRate: number;
  invalidCommitCount: number;
}

export async function runResidualTopKBenchmark(): Promise<ResidualTopKReport> {
...
async function repairOptions(
  ...
  prefix: string,
): Promise<Array<ResidualRepairOption<Parameters<typeof makeReservationCandidate>[0] extends never ? never : unknown>>> {
  const rows: Array<ResidualRepairOption<any>> = [];
