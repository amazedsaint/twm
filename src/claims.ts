...
export const CLAIM_EVIDENCE_GRADES = ["G0", "G1", "G2", "G3"] as const;
export const CLAIM_STATUSES = ["supported", "rejected"] as const;
...
export interface ClaimCertificate {
...
export function requirement(
  key: string,
  passed: boolean,
  options: { reason?: string; evidence?: Record<string, unknown> } = {},
): ClaimRequirement {
  if (!key) {
...
export async function certifyClaim(params: {
  ...
  const pending: ClaimCertificate = {
...
export async function claimCertificateHash(certificate: ClaimCertificate): Promise<string> {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function validateClaimCertificate(certificate: ClaimCertificate): Promise<boolean> {
  try {
...
export function failedClaimKeys(certificate: ClaimCertificate): string[] {
  return certificate.requirements.filter((row) => !row.passed).map((row) => row.key);
}

function validateClaimShape(certificate: ClaimCertificate): void {
