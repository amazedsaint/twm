...
export interface VerifierOnlyAdapter<CandidatePayload> {
  verifierId: string;
  verifierVersion: string;
  verify(candidate: TypedCandidate<CandidatePayload>): Promise<HardVerifierResult> | HardVerifierResult;
}

export class VerifierAgreementAdapter<State, CandidatePayload>
implements ReplayRollbackAdapter<State, CandidatePayload> {
  verifierId: string;
  verifierVersion: string;
  primary: ReplayRollbackAdapter<State, CandidatePayload>;
  audit: VerifierOnlyAdapter<CandidatePayload>;
  primaryCalls = 0;
  auditCalls = 0;
  ...
  constructor(
    primary: ReplayRollbackAdapter<State, CandidatePayload>,
    audit: VerifierOnlyAdapter<CandidatePayload>,
    options: { verifierId?: string; verifierVersion?: string } = {},
  ) {
  ...
  async verify(candidate: TypedCandidate<CandidatePayload>): Promise<HardVerifierResult> {
    this.primaryCalls += 1;
  ...
  async applyCommit(state: State, candidate: TypedCandidate<CandidatePayload>): Promise<State> {
    return this.primary.applyCommit(state, candidate);
  }

  async replay(state: State, receipt: Receipt): Promise<State> {
    return this.primary.replay(state, receipt);
  }

  async rollback(state: State, receipt: Receipt): Promise<State> {
    return this.primary.rollback(state, receipt);
  }

  private resultMatches(result: HardVerifierResult, adapter: VerifierOnlyAdapter<CandidatePayload>): boolean {
    return result.verifierId === adapter.verifierId && result.verifierVersion === adapter.verifierVersion;
  }

  private metadata(
    primaryResult: HardVerifierResult,
    auditResult: HardVerifierResult | undefined,
    auditCalled: boolean,
