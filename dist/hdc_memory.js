...
class ExactContextMemory {
  ...
  rank(exactContext        , labels          , fallback          )           {
...
export async function runHdcMemoryBenchmark(seed = 19, episodes = 96, labelCount = 24)                           {
...
async function runOrder(
  ...
  for (let episode = 0; episode < defects.length; episode += 1) {
    ...
    const state             = { family: lane, episode, solved: false };
...
async function hdcRank(context        , labels          , staticOrder          , memory                        )                    {
...
function makeShapeCandidate(context        , exactContext        , guess        , defect        )                                        {
  return makeCandidate(
    { context, exact_context: exactContext, action: guess, guess, defect }                         ,
    "shape.guess",
    "shape.guess.v1",
  );
}
...
function flipBits(vector           , rate        , seed        )            {
  const rng = mulberry32(seed);
