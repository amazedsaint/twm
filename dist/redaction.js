...
export function makeRedactionPolicy(paths          , policyId = "receipt.redaction")                  {
  return normalizeRedactionPolicy({ schemaVersion: REDACTION_POLICY_SCHEMA, policyId, paths });
}

export function normalizeRedactionPolicy(policy                          )                  {
  const normalized                  = {
...
export async function redactionPolicyHash(policy                 )                  {
  return stableHash(normalizeRedactionPolicy(policy));
}

export async function redactionCommitmentHash(path        , value         , salt        )                  {
  validateRedactionPath(path);
  return stableHash({
...
export async function redactedReceiptHash(view                                               )                  {
  const { redactedHash: _redactedHash, ...withoutHash } = view                           ;
  return stableHash(withoutHash);
}

export async function validateRedactedReceipt(view                                               )                   {
  try {
    if (!isRecord(view)) {
      return false;
    }
    ...
    const policy = normalizeRedactionPolicy(view.redactionPolicy                            );
    ...
    for (const commitment of commitments) {
      const found = getPath(view.redactedPayload                           , commitment.path);
...
export function redactedReceiptCannotReplay(view                                               )          {
  const payload = (view                       ).redactedPayload;
  if (!isRecord(payload)) {
    return true;
  }
...
function validateRedactionPath(path        )       {
...
function redactionMarker(path        , commitmentHash        )                  {
  return {
...
function originalReceiptHashFromPayload(payload                         )         {
...
function coerceCommitment(value         )                      {
  if (!isRecord(value)) {
