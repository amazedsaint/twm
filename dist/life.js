...
export function lifeStep(boardInput       )        {
...
export function* enumerateBoards(height        , width        )                   {
...
export async function makeLifeCandidate(target       , predecessor       , cost        )                                                {
...
export async function guidedLifeTraces(targetInput       , maxCandidates         )                           {
...
export function lifeBaselineVerifierCalls(target       )         {
...
export class LifePredecessorAdapter                                                                   {
...
function blinkerMacroPredecessors(target       )          {
  const board = normalizeBoard(target);
  const height = board.length;
  const width = height === 0 ? 0 : board[0].length;
  const live                          = [];
...
async function reversibleBlinkerTrace(target       , predecessor       , branchId        , cost        )                         {
