...
export async function validateWorldProgramManifest(manifest                      )                   {
  try {
...
export async function validateWorldProgramAdmissionPolicy(policy                             )                   {
  try {
...
export async function validateWorldProgramEvidenceBundle(bundle                            )                   {
  try {
...
export async function validateWorldProgramReplayPackage(replayPackage                           )                   {
  try {
...
export async function worldProgramManifestHash(manifest                      )                  {
  const { manifestHash: _manifestHash, ...withoutHash } = manifest;
  return stableHash(withoutHash);
}

export async function worldProgramAdmissionPolicyHash(policy                             )                  {
  const { policyHash: _policyHash, ...withoutHash } = policy;
  return stableHash(withoutHash);
}

export async function worldProgramCertificateHash(certificate                         )                  {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function worldProgramAdmissionCertificateHash(certificate                                  )                  {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export async function worldProgramEvidenceBundleHash(bundle                            )                  {
...
export async function worldProgramReplayStepHash(step                        )                  {
  const { stepHash: _stepHash, ...withoutHash } = step;
  return stableHash(withoutHash);
}

export async function worldProgramReplayPackageBodyHash(replayPackage                           )                  {
  return stableHash({
    evidenceBundleHash: replayPackage.evidenceBundle.bundleHash,
...
export async function worldProgramReplayPackageHash(replayPackage                           )                  {
...
export async function tamperWorldProgramCertificate(certificate                         )                                   {
...
export async function tamperWorldProgramEvidenceBundle(bundle                            )                                      {
...
export async function tamperWorldProgramReplayPackage(replayPackage                           )                                     {
  const tampered = {
    ...replayPackage,
...
function componentValue(component         , camelName        , snakeName        , fallback        )         {
...
async function bundleInputAttestationHashes(bundle                            )                    {
  return [
...
function allowed(allowedValues          , value        )          {
...
function sameAdmissionCertificate(left                                  , right                                  )          {
  return JSON.stringify(left) === JSON.stringify(right);
}

function sameEvidenceBundle(left                            , right                            )          {
  return JSON.stringify(left) === JSON.stringify(right);
}
...
function sameReplayPackage(left                           , right                           )          {
  return JSON.stringify(left) === JSON.stringify(right);
}
...
function sortedHashGroups(groups                          )                           {
  const result                           = {};
...
function sameHashGroups(left                          , right                          )          {
  return JSON.stringify(sortedHashGroups(left)) === JSON.stringify(sortedHashGroups(right));
}

function isReplayStep(step                                               )                                 {
  return Boolean(
    step
    && typeof step === "object"
    && "schemaVersion" in step
    && (step                               ).schemaVersion === WORLD_PROGRAM_REPLAY_STEP_SCHEMA,
  );
}

async function ledgerHeadFromReplaySteps(steps                          )                  {
  let head = GENESIS_HEAD;
  for (const step of steps) {
    if (step.receipt.parentHead !== head) {
      return "";
