...
export function normalizeAssignment(assignment            , variableCount        )             {
...
export function literalSatisfied(literal        , assignment            )          {
  const value = assignment[Math.abs(literal) - 1];
  return literal > 0 ? value : !value;
}

export function clauseSatisfied(clause        , assignment            )          {
  return clause.some((literal) => literalSatisfied(literal, assignment));
}

export function unsatisfiedClauses(formulaInput            , assignmentInput            )           {
...
export function formulaFromTarget(targetInput            )             {
  const target = normalizeAssignment(targetInput, targetInput.length);
  const clauses           = target.map((value, idx) => [value ? idx + 1 : -(idx + 1)]);
...
export function assignmentFromMask(mask        , variableCount        )             {
...
export async function runStaticSatEpisode(
  ...
  const state           = { formula: normalizeFormula(formula), solved: false, assignment: null };
...
export async function runResidualSatEpisode(
  ...
  const engine = new TransactionEngine(new CnfSatAdapter(), ledger);
  const state           = { formula: normalized, solved: false, assignment: null };
...
export async function runSatCspBenchmark(seed = 23, episodes = 48, variableCount = 8)                        {
