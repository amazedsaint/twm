...
export class BudgetedBranchRuntime                          {
  engine                                            ;
  projector                                          ;
  budget                ;
  ranker                                ;
...
export class DistributedCommitManager                          {
  ...
  async commitOne(state       , workerReceipts                                        )                                {
...
export async function auditBranchSelection(receipts           , certificate                            )                   {
  try {
...
export async function validateBranchSelectionCertificate(certificate                            )                   {
  try {
...
export async function branchSelectionCertificateHash(certificate                            )                  {
  const { certificateHash: _certificateHash, ...withoutHash } = certificate;
  return stableHash(withoutHash);
}

export function candidateVerifierCost(candidate                         , defaultVerifierCost = 1)         {
  if (!Number.isInteger(defaultVerifierCost) || defaultVerifierCost <= 0) {
    throw new RangeError("defaultVerifierCost must be a positive integer");
  }
  const payload = candidate.payload;
  const value = payload && typeof payload === "object"
    ? (payload                           ).verifier_cost ?? (payload                           ).verifierCost ?? defaultVerifierCost
    : defaultVerifierCost;
