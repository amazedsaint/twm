...
export async function runRedactedReceiptBenchmark()                                 {
  ...
  return {
    ...
    visibleVerifierResult: String((view.redactedPayload.hardResult                        ).result),
...
function redactionMarkerPresent(payload                         , path        )          {
  let current          = payload;
  ...
  return Boolean(current && typeof current === "object" && !Array.isArray(current) && (current                          ).redacted === true);
}
